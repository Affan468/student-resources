      ...
      <div className="relative w-full max-w-xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#59a5fb]/15 border border-[#59a5fb]/30 flex items-center justify-center text-[#59a5fb]">
              <BookOpen className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-lg text-slate-900 leading-tight">Add New Engineering Course</h3>
              <p className="text-xs text-slate-500">Populate course directory and student upload options</p>
            </div>
          </div>

          <button 
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center justify-center text-slate-600 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1">
          ...
          {errorMsg && (
            <div className="bg-rose-50 border border-rose-200 rounded-2xl p-3.5 flex items-center gap-3 text-rose-700 text-xs font-semibold animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                required
                className={`w-full px-4 py-3 bg-slate-50 border rounded-xl text-sm font-mono text-slate-900 focus:outline-none focus:ring-2 ${
                  errorMsg ? 'border-rose-300 focus:ring-rose-500' : 'border-slate-200 focus:ring-[#59a5fb]'
                }`}
              />
            </div>
            ...
            {/* Abbreviation */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700">Abbreviation / Alias</label>
              <input
                type="text"
                placeholder="e.g. MSA, DLD, EDC"
                value={formData.abbreviation}
                onChange={(e) => setFormData({ ...formData, abbreviation: e.target.value })}
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#9D00FF]"
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700">Engineering Department / Major</label>
            <select
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-100"
            >
              Cancel
            </button>
            <button
