      ...
      <div className="relative w-full max-w-xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#9D00FF]/15 border border-[#9D00FF]/30 flex items-center justify-center text-[#9D00FF]">
              <UserPlus className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-lg text-slate-900 leading-tight">Add New Faculty Instructor</h3>
              <p className="text-xs text-slate-500">Register new teacher and assign courses</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700">Instructor Full Name *</label>
            <input
              type="text"
              ...
              required
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#9D00FF]"
            />
          </div>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400 font-normal">Select courses this teacher instructs</span>
            </label>
            <div className="max-h-36 overflow-y-auto border border-slate-200 rounded-2xl p-3 bg-slate-50 space-y-1.5">
              {courses.map(course => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-2.5 rounded-xl text-xs font-semibold cursor-pointer transition-colors ${
                      isSelected 
                        ? 'bg-[#9D00FF]/15 border border-[#9D00FF]/30 text-[#9D00FF]'
                        : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-100'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <BookOpen className="w-3.5 h-3.5 text-[#59a5fb]" />
