...
export default function ResourceCard({ resource }) {
  ...
  return (
    <>
      <div className="group relative flex flex-col justify-between rounded-3xl bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/80 hover:border-[#59a5fb] dark:hover:border-[#59a5fb] p-5 shadow-md hover:shadow-xl transition-all duration-300">
        ...
        <div>
          ...
          {/* Title */}
          <h3 className="font-bold text-base text-slate-900 dark:text-white group-hover:text-[#9D00FF] dark:group-hover:text-[#c06eff] transition-colors leading-snug mb-2">
            {resource.title}
          </h3>

          {/* Instructor Badge */}
          {instructor && (
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-700/80 border border-slate-200 dark:border-slate-600 text-xs font-semibold text-slate-700 dark:text-slate-200 mb-3">
              <UserCheck className="w-3.5 h-3.5 text-[#9D00FF] dark:text-[#c06eff]" />
              <span>{instructor.name}</span>
            </div>
          )}

          {/* Metadata info */}
          <div className="flex flex-wrap gap-y-1.5 gap-x-4 text-xs text-slate-500 dark:text-slate-400 mb-6">
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white text-xs font-semibold transition-colors"
          >
