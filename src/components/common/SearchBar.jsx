  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <div className="relative flex items-center">
        <Search className={`absolute left-4 ${isHero ? 'w-5 h-5 text-slate-400' : 'w-4 h-4 text-slate-400'}`} />
        <input
          type="text"
          ...
          className={`w-full ${
            isHero 
              ? 'pl-12 pr-12 py-4 bg-white border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#9D00FF] text-sm shadow-xl'
              : 'pl-10 pr-10 py-2.5 bg-slate-50 hover:bg-slate-100/80 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#59a5fb]'
          } transition-all`}
        />
        {searchQuery && (
          <button
      ...
      {isOpen && query.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden max-h-96 overflow-y-auto divide-y divide-slate-100 animate-fade-in">
          ...
          {hasResults ? (
                    ...
                    {matchedCourses.map((course) => (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <div className="w-9 h-9 rounded-xl bg-[#59a5fb]/15 border border-[#59a5fb]/30 flex items-center justify-center text-[#59a5fb] font-bold text-xs">
                            {course.code}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <h4 className="text-sm font-bold text-slate-900 group-hover:text-[#9D00FF] transition-colors">
                                {course.title}
                              </h4>
                              {course.abbreviation && (
                                <span className="px-2 py-0.5 rounded-md bg-[#9D00FF]/10 text-[#9D00FF] font-mono text-[10px] font-bold">
                                  {course.abbreviation}
                                </span>
                              )}
                            </div>
                            <p className="text-xs text-slate-500">{course.department} • Semester {course.semester}</p>
                          </div>
                        </div>

                        <div className="flex items-center gap-1 text-xs font-semibold text-[#59a5fb] group-hover:text-[#9D00FF]">
                          <span>Open</span>
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">{inst.title} • {inst.department}</p>
                          </div>
                        </div>

                        <div className="flex items-center gap-1 text-xs font-semibold text-[#9D00FF]">
            ...
            <div className="p-6 text-center text-slate-500 text-xs">
              No matching courses or instructors found for "<span className="font-semibold text-slate-800">{searchQuery}</span>".
              <p className="text-[11px] text-slate-400 mt-1">Try searching by abbreviations like OOP, DSA, DBS, OS, SE, CN, AI or course code.</p>
            </div>
          )}
        </div>
      )}
